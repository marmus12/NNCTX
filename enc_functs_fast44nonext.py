...
def get_temps_dests2(ctx_type,ENC=True,nn_model ='dec',ac_model='dec_and_enc',maxesL='dec_and_enc',sess=None,for_train=False,bs_dir=None,save_SSL=True,level=0,ori_level=0,dSSLs=0):
    ...
    # gtLoc = np.copy(Loc)
    wsize = np.sqrt(ctx_type//3).astype(int)
    b = (wsize-1)//2
    ...
    for icPC in range(ncPC+1):
        ...
        if (StartStopLengthM[icPC,2] > 0) & SSL2[icPC] :
            ...
            if for_train:
                Temp, Des = OneSectOctMask2(SLocM,icPC, BWTrue, BWTrue1, BWTrue2, BWTrueM, 0, SectSize, StartStopLengthM,ctx_type,b,ENC,nn_model,ac_model,sess=sess,for_train=for_train)              
                Temps[ iTT:(iTT+Temp.shape[0]),:]  = Temp
                Dests[ iTT:(iTT+Temp.shape[0])]  = Des    
                iTT = iTT+Temp.shape[0]

            else:
                OneSectOctMask2(SLocM,icPC, BWTrue, BWTrue1, BWTrue2, BWTrueM, 0, SectSize, StartStopLengthM,ctx_type,b,ENC,nn_model,ac_model,sess=sess)              
